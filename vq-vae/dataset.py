    ...
    def __init__(self, args, path=None, vocab=None, random=False, split=False, remove_long_words=False):
        ...
        if vocab:
            ...
            for word, freq in zip(self.words, self.freqs):
                for char in word:
                    counter[char] += freq
            self.vocab = ["[1]", "[2]", "[3]", "[PAD]", "[BOS]", "[EOS]", "[BOW]", "[EOW]", "[UNK]"] + [c for c, f in counter.most_common() if f > 0] # alphabet
        ...
        self.continuation_bytes = {i for i, b in enumerate(self.vocab) if type(b)==int and b >= 0x80 and b < 0xC0}
